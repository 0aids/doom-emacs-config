...
(map!
 ...
 :nvm "0" #'evil-beginning-of-visual-line
 :nvm "$"  #'evil-end-of-visual-line)

(setq display-line-numbers-type 'relative)

;; Enable org-fragtog-mode automatically in all org files
(use-package! org-fragtog
  :hook (org-mode . org-fragtog-mode))

(after! org
  ;; (Optional) Make the previews slightly larger for better readability. 
  ;; The default scale is 1.0, which can feel small on high-res monitors.
  (setq org-format-latex-options (plist-put org-format-latex-options :scale 1.5))

  ;; Function to center display equations
  (defun +org-center-latex-previews-a (beg end image &optional imagetype)
    "Center LaTeX fragments that start at the beginning of a line."
    ;; Only apply if the equation is on its own line (display math)
    (when (= beg (line-beginning-position))
      ;; Find the specific overlay generated by org-latex-preview
      (when-let* ((ov (seq-find (lambda (o) (eq (overlay-get o 'org-overlay-type) 'org-latex-overlay))
                                (overlays-at beg)))
                  (disp (overlay-get ov 'display)))
        ;; Push the overlay to the center using line-prefix
        (overlay-put ov 'line-prefix `(space :align-to (- center (0.5 . ,disp)))))))

  ;; Advise the native overlay generation function to apply our centering logic
  (advice-add 'org--make-preview-overlay :after #'+org-center-latex-previews-a))
